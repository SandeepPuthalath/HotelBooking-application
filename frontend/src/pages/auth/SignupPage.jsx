...
const SignupPage = () => {
  ...
  return (
    <div className="relative flex w-screen h-screen justify-center items-center">
      <SignupForm />
      {loading && <Loading />}
      {error && <ErrorMessage error={error} open={error?true: false} onClose ={handleErrorClose}/>}
      {successMessage && <SuccessMessage open={successMessage?true: false} message={SuccessMessage} onClose={handleSuccesClose}/>}
    </div>
  );
};
